...
from utix._util.msg_ext import msg_arg_multi_path_not_exist
from utix._util.path_ext import solve_multi_path
...
from utix.dictex import IndexDict, is_dict__, ListDict, same_key_same_type, merge_tree_leaves_flat_padded, hdict, recursive_getitem, get__
...
class DataInfo:
    ...
    def process_single_data(self, data_entry):
    ...
    def process_data(self, data_entries):
        for data_entry in data_entries:
            self.process_single_data(data_entry)
...
class XDataset(Dataset):
    """
    A dataset object that supports rich options including caching, lazy reading and special support for the `DataEntry` class.
    This class is compatible with PyTorch's dataset loader. The dataset size must be given when using PyTorch's dataset loader.
    """

    def __init__(self, get_item: Callable, transform: Union[Callable, DataInfo] = None, size=None, cache_mode: CacheMode = CacheMode.NoCache, num_workers=1):
        """

        :param get_item:
        :param transform: an object; this object must has a method `process_single_data`
        :param size:
        :param cache_loaded_data:
        """
        self._get_item = get_item
        self._size = size
        self.cache_mode = cache_mode
        self._transform = transform.process_data if hasattr(transform, 'process_data') else transform

        if (self.cache_mode == CacheMode.CacheOnGet and self._size) or (self.cache_mode == CacheMode.CacheAllAtOnce and self._size):
            # data cache is initialized as an empty dictionary for some special cases
        ...
        # region for `CacheMode.CacheAllAtOnce`; loading all data
        if self.cache_mode == CacheMode.CacheAllAtOnce:
            if self._size:
                # dataset size known; supports multi-processing
                if self._num_workers == 1:
                    for i in tqdm(range(self._size), desc='caching data'):
                        self._data_cache[i] = self._get_item(i)
                else:
                    data_items_list, indexes_list = mpex.parallel_process_by_pool(
                        ...
                        target=mpex.MPTarget(target=self._get_item,
                                             ...
                                             remove_none=True),
                        return_job_splits=True)
                    for data_items, indexes in zip(data_items_list, indexes_list):
                        for data_item, index in zip(data_items, indexes):
                            assert self._data_cache[index] is None
                            self._data_cache[index] = data_item
            else:
                # lazy reading; does not support multi-processing
                if not self._data_cache:
                    self._data_cache = list(self._get_item())
            self._size = len(self._data_cache)
    ...
    def get_cache(self):
        if self._cache_complete is True:
            return self._data_cache

    def __getitem__(self, index):
        """
        For lazy-reading, this function returns an iterator.
        :param index:
        :return:
        """
        if self.cache_mode == CacheMode.NoCache:  # BRANCH 1 - no cache, directly retrieves data item
            return self._process_item(self._get_item(index)) if self._size else (self._process_item(item) for item in self._get_item())
        elif self.cache_mode == CacheMode.CacheOnGet:  # BRANCH 2 - data cache enabled
            if self._size:
                # region BRANCH 2.1 - dataset size known; return an item of the specified `index`; this branch is compatible with PyTorch's data loader.
                if self._data_cache[index] is not None:
                    return self._data_cache[index]
                else:
                    self._data_cache[index] = item = self._process_item(self._get_item(index))
                    return item
                # endregion
            else:
                # region BRANCH 2.2 - lazy reading; in this case this method is an iterator
                if self._data_cache is not None:
                    yield from self._data_cache
                else:
                    data_cache = []
                    for item in self._get_item():
                        data_cache.append(self._process_item(item))
                        yield item
                    self._data_cache = data_cache
                    self._cache_complete = True
                # endregion
        elif self.cache_mode == CacheMode.CacheAllAtOnce:
            # BRANCH 3 - data have been loaded
            return self._data_cache[index]

    def __len__(self):
        return self._size  # this will be `None` for lazy reading
...
#
#     def __call__(self, dataset: XDataset, *args, **kwargs):
#         pass

# class FileBasedDataSet(XDataset):
#     def __init__(self, read_file, input_path, file_pattern, max_read, lazy=True, multi_path_delimiter=':', num_p=1):
#         get_item = self.get_item
#         if lazy:
#             pass
#         else:
#             file_paths = solve_multi_path(multi_path=input_path,
#                                           file_pattern=file_pattern,
#                                           multi_path_delimiter=multi_path_delimiter,
#                                           raise_error_if_none_of_the_paths_exist=True,
#                                           remove_non_existent_paths=True)
#             super(FileBasedDataSet, self).__init__(get_item=get_item, size=len(file_paths))
#
...
class HDFS5DataSet(Dataset):
    def __init__(self, h5py_files, meta_files):
        pass
