...
if __name__ == '__main__':
    ...
    lines2 = mpex.mp_read_from_files(num_p=4, file_paths=src_file, chunk_size=1000)

    print(lines1 == lines2)

    lengths1 = tuple(len(x) for x in lines1)
    lengths2 = mpex.mp_read_from_files(num_p=4, target=mpex.MPTarget(target=len, common_func=True, data_from_files=True), file_paths=src_file, chunk_size=1000)
